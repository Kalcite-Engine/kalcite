    ...
    match info.color_type {
        png::ColorType::Rgb => {
            for px in data.chunks_exact(3) {
                pixels.push(rgb565(px[0], px[1], px[2]));
            }
        }
        png::ColorType::Rgba => {
            for px in data.chunks_exact(4) {
                pixels.push(if px[3] < 128 {
                    ...
                    rgb565(px[0], px[1], px[2])
                });
            }
        }
        png::ColorType::Grayscale => {
            for &v in data {
                pixels.push(rgb565(v, v, v));
            }
        }
    ...
    for (path, name) in paths {
            ...
            .as_str()
        {
            ...
            "csv" => {
                ...
                let mut d = Vec::with_capacity(tiles.len() * 2);
                for t in tiles {
...
pub fn encode_pack(entries: &[AssetEntry]) -> Vec<u8> {
    let mut o = Vec::new();
    o.extend(*b"KAP0");
    o.extend((entries.len() as u32).to_le_bytes());
    for e in entries {
        o.extend(e.id.to_le_bytes());
        o.push(e.kind);
        o.extend((e.name.len() as u16).to_le_bytes());
        o.extend((e.data.len() as u32).to_le_bytes());
        o.extend(e.name.as_bytes());
        o.extend(&e.data);
    }
    o
}
