use kalcite_syntax::{Class, Item, Member, TokenKind, lex, parse};
    ...
    if let Ok(tokens) = lex(source) {
        ...
        if saw_asm {
            out.push(Lint {
                ...
                message:
                    "native ASM bypasses Kalcite safety guarantees and is architecture-specific"
                        .into(),
            });
        }
    }
    for item in &module.items {
        match item {
            ...
            Item::Function(function) if function.name.len() == 1 => out.push(Lint {
                ...
                severity: Severity::Warning,
                message: format!(
                    "function `{}` is too terse for a public symbol",
                    function.name
                ),
            }),
            _ => {}
...
fn lint_class(class: &Class, path: &str, out: &mut Vec<Lint>) {
    let is_entity =
        class.attrs.iter().any(|a| a.name == "entity") || class.base.as_deref() == Some("Entity");
    let pool = class.attrs.iter().find(|a| a.name == "pool");
    if is_entity && pool.is_none() {
        out.push(Lint {
            code: "KLC1001",
            severity: Severity::Warning,
            message: format!("entity `{path}` has no explicit @pool capacity"),
        });
    }
    if let Some(pool) = pool {
        match pool.args.first().and_then(|v| v.parse::<usize>().ok()) {
            Some(0) => out.push(Lint {
                code: "KLC1201",
                severity: Severity::Error,
                message: format!("`{path}` has @pool(0); capacity must be at least 1"),
            }),
            Some(n) if n > 4096 => out.push(Lint {
                code: "KLC1202",
                severity: Severity::Warning,
                message: format!(
                    "`{path}` reserves {n} pool slots; this is likely too large for NumWorks"
                ),
            }),
            None => out.push(Lint {
                code: "KLC1203",
                severity: Severity::Error,
                message: format!("`{path}` has an invalid @pool capacity"),
            }),
            _ => {}
        }
    }
...
mod tests {
    use super::*;
    #[test]
    fn nested_entities_are_linted() {
        let l = lint("class G { class Bullet extends Entity { var x:i16; } }");
        assert!(
            l.iter()
                .any(|x| x.code == "KLC1001" && x.message.contains("G.Bullet"))
        );
    }
}
...
mod native_lint_tests {
    ...
    fn native_escape_hatches_are_visible_to_lint() {
        let l = lint(
            r#"@scene class G { fn update() -> void { unsafe rust[desktop] { core::hint::black_box(1u32); } unsafe asm[numworks] { "nop" } } }"#,
        );
