...
fn main() {
    ...
    let mut paddle = 100i16;
    for frame in 0..180 {
        platform.now = frame * 16;
        platform.input = if frame % 80 < 40 {
            Buttons(Buttons::UP)
        } else {
        ...
        if input.held(Buttons::UP) {
            paddle = (paddle - 2).max(0)
        }
        if input.held(Buttons::DOWN) {
            paddle = (paddle + 2).min(H as i16 - 40)
        }
        ball.x += ball.vx;
        ball.y += ball.vy;
        if ball.y <= 0 || ball.y >= H as i16 - 8 {
            ball.vy = -ball.vy
        }
        if ball.x <= 8 && ball.y + 8 >= paddle && ball.y <= paddle + 40 {
            ball.vx = ball.vx.abs()
        }
        if ball.x >= W as i16 - 8 {
            ball.vx = -ball.vx.abs()
        }
        if ball.x < 0 {
            ball.x = 156;
        ...
        let mut c = Canvas::new(&mut framebuffer, W, H).unwrap();
        c.clear(Color565::BLACK);
        c.rect(
            Rect {
                x: 4,
                y: paddle,
