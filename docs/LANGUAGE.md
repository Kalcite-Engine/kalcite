...
Bounded strings expose a portable, allocation-free byte interface through `Text`: `Text.length(value)` returns a `u32`, `Text.byte_at(value, index)` returns a `u8`, and `Text.byte_at_u32(value, index)` returns that byte widened to `u32`. `Text.equals(left, right)` returns a `bool` and accepts bounded strings with different capacities, comparing their used bytes. `Text.starts_with(value, prefix)` compares a bounded prefix without allocating. Their second arguments may be string literals, such as `Text.equals(reference, "local")` or `Text.starts_with(source, "git:")`; the compiler materializes the literal in fixed caller-owned storage. These intrinsic signatures are typechecked, so package code can validate, compare, and hash bounded text without an untyped native escape hatch.
